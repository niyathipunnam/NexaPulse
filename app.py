...
@app.route("/doctor/replies/<int:reply_id>/edit", methods=["POST"])
def doctor_edit_reply(reply_id):
    if "doctor_id" not in session:
        return redirect(url_for("doctor_login"))

    new_text = request.form.get("reply_text")
    if not new_text:
        return redirect(url_for("doctor_dashboard"))
    ...
    cur.execute(
        ...
        """,
        (new_text, reply_id, session["doctor_id"]),
    )
    db.commit()
    cur.close()
...
# =========================
# Patient private chat
# =========================

@app.route("/patient/chat/<int:doctor_id>/<int:question_id>", methods=["GET", "POST"])
def patient_chat(doctor_id, question_id):
    ...
    if request.method == "POST":
        ...
        cur.execute(
            ...
            (patient_id, doctor_id, question_id, sender, message)
            VALUES (%s,%s,%s,'patient',%s)
            """,
            (session["patient_id"], doctor_id, question_id, msg),
        )
        db.commit()
    ...
    cur.execute(
        ...
        ORDER BY created_at
        """,
        (session["patient_id"], doctor_id, question_id),
    )
    ...
    messages = []
    for m in raw_msgs:
        sender_label = "You" if m["sender"] == "patient" else "Doctor"
        created_at_display = m["created_at"].strftime("%d %b %Y %I:%M %p")
        from_doctor = (m["sender"] == "doctor")
        messages.append({
...
@app.route("/doctor/chat/<int:patient_id>/<int:question_id>", methods=["GET", "POST"])
def doctor_chat(patient_id, question_id):
    ...
    if request.method == "POST":
        ...
        cur.execute(
            ...
            """,
            (patient_id, session["doctor_id"], question_id, msg),
        )
        db.commit()
    ...
    cur.execute(
        ...
        ORDER BY created_at
        """,
        (patient_id, session["doctor_id"], question_id),
    )
    raw_msgs = cur.fetchall()
    cur.close()
    ...
    messages = []
    for m in raw_msgs:
        sender_label = "You" if m["sender"] == "doctor" else "Patient"
        created_at_display = m["created_at"].strftime("%d %b %Y %I:%M %p")
        from_doctor = (m["sender"] == "doctor")
        messages.append({
            "sender_label": sender_label,
